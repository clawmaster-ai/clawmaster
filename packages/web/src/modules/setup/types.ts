...
export const PRIMARY_PROVIDERS = ['baidu-aistudio', 'openai', 'anthropic', 'google', 'deepseek', 'ollama'] as const

export const PROVIDER_BADGES = {
  'baidu-aistudio': 'golden-sponsor',
} as const

export function getProviderCredentialLabel(providerId: string, _language?: string): string {
  if (providerId === 'baidu-aistudio') {
    return 'Access Token'
  }
  return 'API Key'
}
