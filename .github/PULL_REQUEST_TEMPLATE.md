## What

<!-- What changed? One paragraph. Required — PR check will fail if this is empty. -->

## Why

<!-- Why is this needed? What problem does it solve? -->

## How

<!-- How was it implemented? Note non-obvious design decisions. Skip if self-evident from the diff. -->

## Related Issue

<!-- Closes #NNN -->

## Type of Change

- [ ] Bug fix
- [ ] New feature
- [ ] Refactor (no behavior change)
- [ ] Documentation
- [ ] Tests
- [ ] CI / tooling
- [ ] Chore (dependencies, config)

## Testing

- [ ] `npm test` passes locally
- [ ] New or modified behavior has unit tests
- [ ] No `console.log` or debug artifacts left in code

## Dependency Changes

- [ ] No new non-Node.js runtime dependencies introduced
- [ ] New npm packages (if any) are justified in the Summary above and have an open issue

## Checklist

- [ ] PR is a **draft** if not yet ready for review
- [ ] Branch name follows convention (`feat/`, `fix/`, `chore/`, `docs/`, `test/`, `ci/`)
- [ ] All new i18n strings added to `packages/web/src/locales/main/{zh,en,ja}.ts`

> [!CAUTION]
> This PR does **not** contain: screenshots, screen recordings, test output logs, generated files (`dist/`, `coverage/`), or `console.log` debug calls. Delete this line when confirmed.
