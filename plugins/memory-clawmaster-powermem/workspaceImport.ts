...
export function resolveOpenclawWorkspaceDir(): string {
...
async function listWorkspaceMemoryDocuments(): Promise<WorkspaceMemoryDocument[]> {
  ...
  for (const sourcePath of dedupedPaths) {
    ...
    if (!content) continue
    documents.push({
      id: `${DEFAULT_AGENT_ID}:${sourcePath}`,
