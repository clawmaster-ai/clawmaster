...
test('formatAutoRecallLog summarizes wiki relevance and hit counts', () => {
  assert.equal(
    formatAutoRecallLog('what do we know about AI agents?', { wikiCount: 2, memoryCount: 1 }),
    'memory-clawmaster-powermem: auto-recall wikiRelevant=true wikiHits=2 memoryHits=1 query="what do we know about AI agents?"',
  )
  assert.equal(
    formatAutoRecallLog('what is 2 plus 2?', { wikiCount: 0, memoryCount: 0 }),
